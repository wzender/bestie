...
def render_tab_content(tab, selected_rows, histogram_click_data):
    if not selected_rows:
    ...
    if (
        ctx.triggered
        and ctx.triggered[0]["prop_id"] == "histogram-click-data.data"
    ...
    if tab == "error-analysis":
        ...
        try:
            ...
            cm_fig.update_layout(
                ...
                yaxis={"fixedrange": True},
                width=None,  # Allow figure to stretch to column width
                height=400,
                font=dict(size=10),
        ...
        content = [
            ...
            # Confusion Matrix and Datapoint Table side by side (each half the screen)
            dbc.Row(
                [
                    dbc.Col(
                        [
