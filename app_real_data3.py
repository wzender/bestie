...
def generate_mock_data():
    ...
    for run_idx, run_id in enumerate(run_data["run_id"][:-1]):  # Exclude test run
        ...
        # Collect subtypes for selected types
        run_subtypes_pool = []
        for t in run_types:
            type_subtypes = [s for s in all_subtypes if s.startswith(t)]
            run_subtypes_pool.extend(type_subtypes)

        # Select 10–20 unique subtypes for this run
        num_subtypes = np.random.randint(10, 21)
        run_subtypes = np.random.choice(
            run_subtypes_pool, min(num_subtypes, len(run_subtypes_pool)), replace=False
        )
        ...
        # Assign true_sub_type and pred_sub_type from run_subtypes
        run_data_chunk["true_sub_type"] = np.random.choice(run_subtypes, rows_per_run)
        run_data_chunk["pred_sub_type"] = np.random.choice(run_subtypes, rows_per_run)
