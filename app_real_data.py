...
detailed_data["true_sub_type"] = detailed_data["true_type"].apply(
    lambda t: np.random.choice([s for s in all_subtypes if s.startswith(t)])
)
# Assign pred_sub_type based on true_type
...
# Assign pred_unk_subtype with same distribution as test_run_data
np.random.seed(43)
detailed_data["pred_unk_subtype"] = np.random.choice(
    unk_subtypes, size=len(detailed_data), p=[0.09] * 10 + [0.1]
)
