...
all_subtypes = [f"{t}{i}" for t in all_types for i in [1, 2]]  # ["A1", "A2", "B1", "B2", ..., "T1", "T2"]
...
detailed_data['true_sub_type'] = detailed_data['true_type'].apply(lambda t: np.random.choice([f"{t}1", f"{t}2"]))
detailed_data['pred_sub_type'] = detailed_data['true_type'].apply(lambda t: np.random.choice([f"{t}1", f"{t}2"]))
detailed_data['correct'] = detailed_data['true_sub_type'] == detailed_data['pred_sub_type']
...
app.layout = dbc.Container([
        ...
        dbc.Col([
            ...
            dash_table.DataTable(
                ...
                columns=[
                    ...
                    {'name': 'F1 Score', 'id': 'f1_score', 'type': 'numeric', 'format': {'specifier': '.3f'}}
                ],
                data=run_data.to_dict('records'),
                style_table={'overflowX': 'auto', 'maxHeight': '33vh', 'overflowY': 'auto'},
    ...
    dbc.Row([
        dbc.Col([
            ...
            dbc.Row([
                dbc.Col([
                    html.H3(id='run-analysis-title', className="mt-2 mb-2")
                ], md=8),
                dbc.Col([
                    dbc.Checklist(
                        id='show-mistakes-switch',
                        options=[{'label': 'Show Mistakes Only', 'value': 1}],
                        value=[],
                        switch=True,
                        className="mt-2 mb-2",
                        style={'fontSize': '0.9rem', 'display': 'none'}  # Compact and initially hidden
                    )
                ], md=4, className="text-end")
            ]),
            dcc.Tabs(id='analysis-tabs', value='error-analysis', children=[
...
@app.callback(
    Output('show-mistakes-switch', 'style'),
    [Input('analysis-tabs', 'value')]
)
def toggle_switch_visibility(tab):
    base_style = {'fontSize': '0.9rem'}
...
def render_tab_content(tab, selected_rows, histogram_click_data, show_mistakes):
    ...
    if triggered_id == 'histogram-click-data' and histogram_click_data and 'points' in histogram_click_data:
    ...
    type_counts = run_data_filtered['true_type'].value_counts().reindex(all_types, fill_value=0).reset_index()
    type_counts.columns = ['true_type', 'count']
    type_fig = px.bar(type_counts, x='true_type', y='count', title='True Type Distribution')
    ...
    if selected_true_type:
        selected_index = all_types.index(selected_true_type) if selected_true_type in all_types else None
        type_fig.update_traces(selectedpoints=[selected_index] if selected_index is not None else [])
    ...
    if selected_true_type and selected_true_type in all_types:
        cm_table_data = run_data_filtered[run_data_filtered['true_type'] == selected_true_type]
    ...
    if tab == 'error-analysis':
        ...
        true_subtypes_with_preds = [subtype for subtype in true_subtypes if subtype in cm.index and cm.loc[subtype].sum() > 0]
        ...
        if selected_true_type:
            relevant_subtypes = [s for s in relevant_subtypes if s.startswith(selected_true_type)]
        if not relevant_subtypes:
            possible_subtypes = [f"{selected_true_type}{i}" for i in [1, 2]] if selected_true_type else all_subtypes
            relevant_subtypes = [s for s in possible_subtypes if s in pred_subtypes or s in true_subtypes_with_preds]
            if not relevant_subtypes:
                content = type_histogram_content + [
                    dbc.Row([
                        dbc.Col([
        ...
        cm = cm.reindex(index=relevant_subtypes, columns=relevant_subtypes, fill_value=0)
        ...
        try:
            cm_fig = ff.create_annotated_heatmap(
                z, x=labels, y=labels, annotation_text=z_text, colorscale='Blues',
                showscale=False
            )
...
def update_datapoint_table(confusion_click_data, selected_rows, selected_true_type, show_mistakes):
    if not selected_rows:
        return []
    ...
    if selected_true_type and selected_true_type in all_types:
        run_data_filtered = run_data_filtered[run_data_filtered['true_type'] == selected_true_type]
    else:
        return []  # No data if no type is selected
