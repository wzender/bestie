...
def generate_mock_data():
    ...
    for run_idx, run_id in enumerate(run_data["run_id"][:-1]):  # Exclude test run
        ...
        for t in run_types:
            ...
            run_subtypes.extend(selected_subtypes)

        # Ensure total subtypes are between 10 and 20
        if len(run_subtypes) > 20:
            run_subtypes = np.random.choice(run_subtypes, 20, replace=False)
        elif len(run_subtypes) < 10:
            # If fewer than 10, add more subtypes from run_types
            ...
            if run_subtypes_pool:  # Only if there are available subtypes
                additional_subtypes = np.random.choice(
                    run_subtypes_pool,
                    min(additional_needed, len(run_subtypes_pool)),
                    replace=False,
                )
                run_subtypes = np.concatenate([run_subtypes, additional_subtypes])

        # Convert to list if necessary
        run_subtypes = list(run_subtypes)
...
def render_subtype_confusion_content(highlighted_run_id, histogram_click_data):
    ...
    # Create confusion matrix using px.imshow
    try:
        cm_fig = px.imshow(
            cm,
            text_auto=True,
            color_continuous_scale="Blues",
            labels=dict(x="Predicted Subtype", y="True Subtype"),
            x=cm.columns.tolist(),
            y=cm.index.tolist(),
        )
        cm_fig.update_layout(
            ...
            xaxis={"fixedrange": True, "tickangle": 45},
            yaxis={"fixedrange": True},
            # width=1400,  # Full screen width
            # height=400,  # Unchanged height
            font=dict(size=10),
