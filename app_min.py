...
# Callback to filter leaderboard table based on benchmark selection
@app.callback(
    Output("leaderboard-table", "data"), [Input("benchmark-dropdown", "value")]
)
def filter_leaderboard_by_benchmark(selected_benchmark):
    if not selected_benchmark:
        return []  # Show no data if no benchmark is selected
