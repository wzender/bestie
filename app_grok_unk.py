...
detailed_data["pred_unk_subtype"] = None  # Non-test runs get None
...
app.layout = dbc.Container(
    [
        ...
        # Store for histogram click data (for Subtype confusion by type)
        ...
        # Store for pred_unk_subtype histogram click data
        dcc.Store(id="pred-unk-subtype-click-data", data=None),
    ],
    fluid=True,
)


# Callback to highlight table row
@app.callback(
    [
    ...
    [
        ...
        Input("leaderboard-table", "page_size"),
    ],
    [State("leaderboard-table", "derived_viewport_data")],
    prevent_initial_call=True,
)
def highlight_table_row(active_cell, page_current, page_size, viewport_data):
    if not active_cell or not viewport_data:
        return dash.no_update, dash.no_update

    # Get the run_id from the clicked row in the current page's data
    row_index = active_cell["row"]
    highlighted_run_id = viewport_data[row_index]["run_id"]

    # Update style_data_conditional to highlight the selected run_id
    ...
    return highlighted_run_id, style_data_conditional
...
@app.callback(
    Output("run-analysis-title", "children"), [Input("highlighted-run-id", "data")]
)
def update_run_analysis_title(highlighted_run_id):
    if not highlighted_run_id:
        return "Run Analysis"
    return f"{highlighted_run_id} Run Analysis"


# Callback to capture type-histogram click data (for Subtype confusion by type)
@app.callback(
...
def capture_histogram_click_data(click_data, highlighted_run_id, tab):
    ctx = callback_context
    if not ctx.triggered or not highlighted_run_id or tab != "subtype-confusion-type":
        return None
...
# Callback to capture pred_unk_subtype histogram click data (for Unknown Subtype Histogram)
@app.callback(
    Output("pred-unk-subtype-click-data", "data"),
    [
        Input("pred-subtype-histogram", "clickData"),
        Input("highlighted-run-id", "data"),
        Input("analysis-tabs", "value"),
    ],
    prevent_initial_call=True,
)
def capture_pred_unk_subtype_click_data(click_data, highlighted_run_id, tab):
    ctx = callback_context
    if (
        not ctx.triggered
        or not highlighted_run_id
        or tab != "unknown-subtype-histogram"
    ):
        return None

    triggered_id = ctx.triggered[0]["prop_id"].split(".")[0]
    if (
        triggered_id == "pred-subtype-histogram"
        and click_data
...
def render_tab_content(
    tab, highlighted_run_id, histogram_click_data, show_mistakes, top_n
):
    if not highlighted_run_id:
    ...
    if tab == "subtype-confusion-type":
        ...
        # Predicted Unknown Subtype Histogram (show only subtypes with existing records)
        pred_subtype_counts = (
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.H4(
                                "Predicted Unknown Subtype Histogram", className="mb-1"
                            ),
                            dcc.Graph(
...
def update_datapoint_table(
    confusion_click_data, highlighted_run_id, selected_true_type, show_mistakes
):
    if not highlighted_run_id:
        return []
...
def update_subtype_datapoint_table(confusion_click_data, highlighted_run_id, top_n):
    if not highlighted_run_id:
        return dbc.Alert(
...
# Callback to update unk subtype datapoint table based on histogram clicks (Unknown Subtype Histogram)
@app.callback(
    Output("unk-subtype-datapoint-table-container", "children"),
    [Input("pred-unk-subtype-click-data", "data"), Input("highlighted-run-id", "data")],
    prevent_initial_call=True,
)
def update_unk_subtype_datapoint_table(click_data, highlighted_run_id):
    if not highlighted_run_id:
        return dbc.Alert(
            "Highlight a run from the leaderboard",
            color="info",
    ...
    selected_unk_subtype = click_data["points"][0]["x"]
    # Handle None case (stored as None in DataFrame, but may appear as 'None' in clickData)
    selected_unk_subtype = (
        None if selected_unk_subtype == "None" else selected_unk_subtype
    )

    filtered = run_data_filtered[
        run_data_filtered["pred_unk_subtype"] == selected_unk_subtype
    ]

    if filtered.empty:
        return dbc.Alert(
            f"No records found for Pred Unk Subtype: {selected_unk_subtype or 'None'}",
            color="info",
            className="text-center",
        )

    return dash_table.DataTable(
        id="unk-subtype-datapoint-table",
        columns=[
            {"name": "Text", "id": "text"},
            {"name": "True Type", "id": "true_type"},
            {"name": "True Subtype", "id": "true_sub_type"},
