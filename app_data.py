...
# Define types and subtypes for test run
test_types = ["Perfect", "WorstMin", "Medium"]
test_subtypes = {
    "Perfect": [f"Perfect{i}" for i in range(1, 11)],  # Perfect1 to Perfect10
    "WorstMin": [f"WorstMin{i}" for i in range(1, 11)],  # WorstMin1 to WorstMin10
    "Medium": [f"Medium{i}" for i in range(1, 11)],  # Medium1 to Medium10
}
all_test_subtypes = (
    test_subtypes["Perfect"]
    + test_subtypes["WorstMin"]
    + test_subtypes["Medium"]
    + ["Else"]
)

# Test run data with increased variety
np.random.seed(43)
n_test_samples = 100
test_run_data = pd.DataFrame(
    {
        "Text": [f"Text{i+1}" for i in range(n_test_samples)],
        "true_type": np.random.choice(test_types, n_test_samples, p=[0.33, 0.33, 0.34]),
    }
)
# Assign true_sub_type based on true_type
test_run_data["true_sub_type"] = test_run_data["true_type"].apply(
    lambda t: np.random.choice(test_subtypes[t])
)
# Assign pred_sub_type from all possible subtypes, allowing cross-type predictions
test_run_data["pred_sub_type"] = np.random.choice(
    all_test_subtypes, n_test_samples, p=[0.033] * 30 + [0.01]
)
# Set correct column
...
# Define types and subtypes for the app
all_types = [chr(65 + i) for i in range(20)] + ["Perfect", "WorstMin", "Medium"]
all_subtypes = [f"{t}{i}" for t in all_types[:20] for i in [1, 2]] + all_test_subtypes[
    :-1
]  # Exclude 'Else'
...
# Callback to filter leaderboard table and highlight first row based on benchmark selection
@app.callback(
    [
...
def filter_leaderboard_by_benchmark(selected_benchmark):
    if not selected_benchmark:
        return [], None, [], None, None

    # Filter leaderboard data for the selected benchmark
    df = run_data[run_data["benchmark"] == selected_benchmark]
    data = df.to_dict("records")

    # Get the run_id of the first row (if data exists)
    highlighted_run_id = df.iloc[0]["run_id"] if not df.empty else None

    # Define style to highlight the first row
    style_data_conditional = (
        ...
        if highlighted_run_id
        else []
    )

    # Reset analysis state
    selected_true_type = None
    histogram_click_data = None
    ...
    return (
        ...
        style_data_conditional,
        selected_true_type,
        histogram_click_data,
    )


# Callback to highlight table row on click
@app.callback(
    [
        Output("highlighted-run-id", "data"),
        Output("leaderboard-table", "style_data_conditional"),
...
def highlight_table_row(active_cell, page_current, page_size, viewport_data):
    ...
    # Only update if triggered by active_cell (row click)
    if (
        triggered_id != "leaderboard-table.active_cell"
        or not active_cell
        or not viewport_data
    ):
